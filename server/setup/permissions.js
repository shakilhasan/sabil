...
const seed = async (logger) => {
  ...
  await Promise.all(data.map(async (item) => {
    logger.info(`Checking if permission ${item.resourceName} for ${
        item.roleName} exists`);
    ...
    if (!itemExists) {
      ...
      const resource =
          await searchOne({name : item.resourceName}, resourceModel);
      try {
      ...
      const updatedItem =
          await update({_id : itemExists._id, ...item}, permissionModel);
      logger.info(`Permission ${item.resourceName} for ${item.roleName} of id ${
          updatedItem._id} is updated`);
    }
  }));
...
const migrate = async (logger) => {
  ...
  const superadminUser =
      await searchOne({username : "superadmin@sabil.com"}, "User");
  if (!superadminUser) {
