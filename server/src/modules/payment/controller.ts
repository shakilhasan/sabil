...
const initHandler = async (req:any, res:any, next:any) => {
    const body = req.body;
    const data = {
...
const validateHandler = async (req:any, res:any, next:any) => {
    const body = req.body;
...
const initiateRefundHandler = async (req:any, res:any, next:any) => {
    const body = req.body;
    const data = {
    ...
    const sslcz = new SSLCommerzPayment(store_id, store_passwd, is_live)
    sslcz.initiateRefund(data).then((data:any) => {
        //process the response that got from sslcommerz
...
const refundQueryHandler = async (req:any, res:any, next:any) => {
    const body = req.body;
    const data = {
        refund_ref_id: body.refund_ref_id //"SL4561445410",
    };
    const sslcz = new SSLCommerzPayment(store_id, store_passwd, is_live)
    sslcz.refundQuery(data).then((data:any) => {
        //process the response that got from sslcommerz
        //https://developer.sslcommerz.com/doc/v4/#initiate-the-refund
        return res.status(200).send({data});
    });
...
const transactionQueryByTransactionIdHandler = async (req:any, res:any, next:any) => {
    const body = req.body;
    const data = {
        tran_id: body.tran_id //AKHLAKJS5456454,
    };
    const sslcz = new SSLCommerzPayment(store_id, store_passwd, is_live)
    sslcz.transactionQueryByTransactionId(data).then((data:any) => {
        //process the response that got from sslcommerz
...
//you also use this as internal method
const transactionQueryBySessionIdHandler = async (req:any, res:any, next:any) => {
    const body = req.body;
    const data = {
        sessionkey: body.sessionkey //AKHLAKJS5456454,
    };
    const sslcz = new SSLCommerzPayment(store_id, store_passwd, is_live);
    sslcz.transactionQueryBySessionId(data).then((data:any) => {
        //process the response that got from sslcommerz
        //https://developer.sslcommerz.com/doc/v4/#by-session-id
        return res.status(200).send({data});
    });
...
paymentRouter.post("/init", initHandler);
paymentRouter.post("/validate", validateHandler);
paymentRouter.post("/initiate-refund", initiateRefundHandler);
paymentRouter.post("/refund-query", refundQueryHandler);
paymentRouter.post("/transaction-query-by-transaction-id", transactionQueryByTransactionIdHandler);
paymentRouter.post("/transaction-query-by-session-id", transactionQueryBySessionIdHandler);
