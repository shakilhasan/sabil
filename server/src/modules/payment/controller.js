...
// sslcommerz validation, Validate after successful transaction (inside success
// and ipn controller methods)
const validateHandler = async (req, res, next) => {
  const body = req.body;

  const data = {
    val_id :
        body.val_id //'ADGAHHGDAKJ456454' //that you go from sslcommerz response
  };
  const sslcz = new SSLCommerzPayment(store_id, store_passwd, is_live);
...
// SSLCommerz transactionQueryByTransactionId, Query the status of a transaction
// (by Transaction ID) you also use this as internal method
...
// SSLCommerz transactionQueryBySessionId, Query the status of a transaction (by
// session ID) you also use this as internal method
...
router.post("/transaction-query-by-transaction-id",
            transactionQueryByTransactionIdHandler);
router.post("/transaction-query-by-session-id",
            transactionQueryBySessionIdHandler);
