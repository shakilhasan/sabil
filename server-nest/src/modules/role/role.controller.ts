...
export class RoleController {
  ...
  async createRole(@Body() createRoleDto: CreateRoleDto, @Res() res: Response) {
    ...
    try {
      const newRole: any = await this.roleService.createRole(
        createRoleDto,
        session,
      );
  ...
  async updateRole(@Body() updateRoleDto: UpdateRoleDto, @Res() res: Response) {
    ...
    session.startTransaction();
    try {
      const newRole: any = await this.roleService.updateRole(
        updateRoleDto,
        session,
      );
      await session.commitTransaction();
  ...
  async deleteRole(@Query() query: Role, @Res() res: Response) {
    ...
    session.startTransaction();
    try {
      const deletedRole: any = await this.roleService.deleteRole(
        query.id,
        session,
      );
      await session.commitTransaction();
